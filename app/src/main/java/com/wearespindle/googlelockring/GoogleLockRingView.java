...
public class GoogleLockRingView extends View {
    private static final String TAG = "GoogleLockRingView";
    ...
    public GoogleLockRingView(Context context) {
        this(context, null);
    }

    public GoogleLockRingView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GoogleLockRingView);
        mInnerRadius = a.getDimension(R.styleable.GoogleLockRingView_innerRadius, mInnerRadius);
        mOuterRadius = a.getDimension(R.styleable.GoogleLockRingView_outerRadius, mOuterRadius);
        mSnapMargin = a.getDimension(R.styleable.GoogleLockRingView_snapMargin, mSnapMargin);
        mVibrationDuration = a.getInt(R.styleable.GoogleLockRingView_vibrationDuration, mVibrationDuration);
        mFeedbackCount = a.getInt(R.styleable.GoogleLockRingView_feedbackCount, mFeedbackCount);
        TypedValue handle = a.peekValue(R.styleable.GoogleLockRingView_handleDrawable);
        mHandleDrawable = new TargetDrawable(res, handle != null ? handle.resourceId : 0, 2);
        mHandleDrawable.setState(TargetDrawable.STATE_INACTIVE);
        mOuterRing = new TargetDrawable(res, getResourceId(a, R.styleable.GoogleLockRingView_outerRingDrawable), 1);

        mAlwaysTrackFinger = a.getBoolean(R.styleable.GoogleLockRingView_alwaysTrackFinger, false);

        int pointId = getResourceId(a, R.styleable.GoogleLockRingView_pointDrawable);
        Drawable pointDrawable = pointId != 0 ? res.getDrawable(pointId) : null;
        mGlowRadius = a.getDimension(R.styleable.GoogleLockRingView_glowRadius, 0.0f);

        TypedValue outValue = new TypedValue();

        // Read array of target drawables
        if (a.getValue(R.styleable.GoogleLockRingView_targetDrawables, outValue)) {
        ...
        if (a.getValue(R.styleable.GoogleLockRingView_targetDescriptions, outValue)) {
        ...
        if (a.getValue(R.styleable.GoogleLockRingView_directionDescriptions, outValue)) {
            final int resourceId = outValue.resourceId;
            if (resourceId == 0) {
        ...
        mGravity = a.getInt(R.styleable.GoogleLockRingView_android_gravity, Gravity.TOP);
        a.recycle();
    ...
    private void internalSetTargetResources(int resourceId) {
        Log.e(TAG, Integer.toString(resourceId));
