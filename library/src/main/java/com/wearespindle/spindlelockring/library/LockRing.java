package com.wearespindle.spindlelockring.library;
...
public class LockRing extends View {
    private static final String TAG = "LockRing";
    ...
    public LockRing(Context context) {
        this(context, null);
    }

    public LockRing(Context context, AttributeSet attrs) {
        super(context, attrs);

        Resources res = context.getResources();

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LockRing);
        mInnerRadius = a.getDimension(R.styleable.LockRing_innerRadius, mInnerRadius);
        mOuterRadius = a.getDimension(R.styleable.LockRing_outerRadius, mOuterRadius);
        mSnapMargin = a.getDimension(R.styleable.LockRing_snapMargin, mSnapMargin);
        mVibrationDuration = a.getInt(R.styleable.LockRing_vibrationDuration,
                mVibrationDuration);
        mFeedbackCount = a.getInt(R.styleable.LockRing_feedbackCount,
                mFeedbackCount);
        TypedValue handle = a.peekValue(R.styleable.LockRing_handleDrawable);
        mHandleDrawable = new TargetDrawable(res, handle != null ? handle.resourceId : 0, 2);
        mHandleDrawable.setState(TargetDrawable.STATE_INACTIVE);
        mOuterRing = new TargetDrawable(
                res,
                getResourceId(a,
                        R.styleable.LockRing_outerRingDrawable),
                1
        );

        mAlwaysTrackFinger = a.getBoolean(R.styleable.LockRing_alwaysTrackFinger, false);

        int pointId = getResourceId(a, R.styleable.LockRing_pointDrawable);
        Drawable pointDrawable = pointId != 0 ? res.getDrawable(pointId) : null;
        mGlowRadius = a.getDimension(R.styleable.LockRing_glowRadius, 0.0f);

        TypedValue outValue = new TypedValue();

        // Read array of target drawables.
        if (a.getValue(R.styleable.LockRing_targetDrawables, outValue)) {
        ...
        if (a.getValue(R.styleable.LockRing_targetDescriptions, outValue)) {
        ...
        if (a.getValue(R.styleable.LockRing_directionDescriptions, outValue)) {
            final int resourceId = outValue.resourceId;
            if (resourceId == 0) {
        ...
        mGravity = a.getInt(R.styleable.LockRing_android_gravity, Gravity.TOP);
        a.recycle();
    ...
    protected int getSuggestedMinimumWidth() {
        // View should be large enough to contain the background + handle and target
        // drawable on either edge.
        return (int) (Math.max(mOuterRing.getWidth(), 2 * mOuterRadius) + mMaxTargetWidth);
    }

    @Override
    protected int getSuggestedMinimumHeight() {
        // View should be large enough to contain the unlock ring + target and target
        // drawable on either edge
    ...
    private ArrayList<TargetDrawable> loadDrawableArray(int resourceId) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            TargetDrawable target = new TargetDrawable(
                    res,
                    value != null ? value.resourceId : 0,
                    3
            );
     ...
     *
     * @param enabled if the vibrater is enabled or not.
     */
    ...
    private void startBackgroundAnimation(int duration, float alpha) {
        ...
        if (mAlwaysTrackFinger && background != null) {
            ...
            mBackgroundAnimator = Tweener.to(
                    background,
                    duration,
                    "ease",
                    Ease.Cubic.easeIn,
